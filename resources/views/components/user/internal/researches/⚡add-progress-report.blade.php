...
new class extends Component {
    ...
    // ── Existing files (untuk ditampilkan saat edit) ──
    ...
    {
        // Saat edit: file boleh kosong kalau sudah ada file lama
        $reportRule = $this->editMode ? ['nullable', 'file', 'mimes:pdf', 'max:10240'] : ['required', 'file', 'mimes:pdf', 'max:10240'];

        $pptRule = $this->editMode ? ['nullable', 'file', 'mimes:ppt,pptx', 'max:20480'] : ['required', 'file', 'mimes:ppt,pptx', 'max:20480'];

        return [
            'progressSummary' => ['required', 'string', 'min:20'],
            'progressKeyword' => ['required', 'string', 'max:255'],
            'progressReportFile' => $reportRule,
            'progressPptFile' => $pptRule,
        ];
    }
    ...
    public function with(): array
    {
        ...
        if ($this->proposal) {
            $progressReports = ProgressReport::where('proposal_id', $this->proposal->id)
                ->with(['reviewer', 'notes.reviewer'])
                ->latest()
                ->get();
        }

        return compact('progressReports');
    }
    ...
    public function openEditForm(int $id): void
    {
        ...
        $report = ProgressReport::where('proposal_id', $this->proposal->id)->findOrFail($id);

        // Guard: hanya boleh edit saat pending atau rejected
        if (!$report->canBeEdited()) {
            Flux::toast(text: 'This report is currently under review or already approved and cannot be edited.', variant: 'danger');
            return;
        }
        ...
        $this->progressKeyword = $report->keyword;
        $this->progressSummary = $report->summary;
    ...
    public function store(): void
    {
        ...
        // Upload report PDF kalau ada file baru
        if ($this->progressReportFile) {
            $data['report_path'] = $this->progressReportFile->store("progress-reports/{$this->proposal->id}", 'public');
        }

        // Upload PPT kalau ada file baru
        if ($this->progressPptFile) {
            $data['ppt_path'] = $this->progressPptFile->store("progress-reports/{$this->proposal->id}", 'public');
        }

        // ═════ UPDATE MODE ═════
        if ($this->editMode && $this->editingId) {
            $report = ProgressReport::where('proposal_id', $this->proposal->id)->findOrFail($this->editingId);

            if (!$report->canBeEdited()) {
                Flux::toast(text: 'This report can no longer be edited.', variant: 'danger');
                return;
            }
            ...
            // Setelah author resubmit:
            // - reviewer_id DIPERTAHANKAN (reviewer yang sama review ulang)
            // - reviewed_at = null → status kembali "Under Review"
            // - is_approved = false
            ...
            Flux::toast(text: 'Progress report updated and resubmitted for review.', variant: 'success');
            $this->dispatch('added-success', message: 'Progress report updated.');
        }
        // ═════ CREATE MODE ═════
        else {
    ...
    public function deleteReport(int $id): void
    {
        ...
        if (!$report->canBeDeleted()) {
            Flux::toast(text: 'This report has been assigned to a reviewer and can no longer be deleted.', variant: 'danger');
            return;
        }
...
<div x-data="{
    ...
    class="fixed inset-0 z-100 flex items-end sm:items-center justify-center p-0 sm:p-4" @click.self="close()">
                            ...
                            <div
                                ...
                                <div class="flex items-start justify-between gap-2 mb-1.5">
                                    ...
                                    <div class="flex items-center gap-0.5 shrink-0">
                                        {{-- Edit: hanya saat pending atau rejected --}}
                                        @if ($report->canBeEdited())
                                            <button type="button" wire:click="openEditForm({{ $report->id }})"
                                                title="Edit & Resubmit"
                                ...
                                <div
                                    class="mt-2 pt-2 flex flex-wrap items-center gap-3
                                    ...
                                    @endif
                                    @if ($report->ppt_path)
                                        <a href="{{ Storage::disk('public')->url($report->ppt_path) }}" target="_blank"
                                            class="inline-flex items-center gap-1 text-[10px]
                                ...
                                @if ($report->notes->isNotEmpty())
                                        <div
                                            class="mt-2 pt-2 border-t border-outline-variant/40 dark:border-zinc-700/60">
                                            <div
                                                class="text-[10px] font-semibold uppercase tracking-wider
                    text-outline dark:text-zinc-500 mb-1.5">
                                                Reviewer Feedback
                                            </div>

                                            @foreach ($report->notes as $note)
                                                @php $nMeta = $note->decisionMeta(); @endphp
                                                <div
                                                    class="mb-1.5 last:mb-0 p-2 rounded-md
                        bg-white/60 dark:bg-zinc-900/40
                        border border-outline-variant/40 dark:border-zinc-700/60">
                                 ...
                                 {{ $nMeta['class'] }}">
                                                        ...
                                                        <span
                                                            class="text-[9px] text-outline dark:text-zinc-500 font-mono">
                          ...
                          text-on-surface-variant dark:text-zinc-400">
                                                    ...
                                                    @if ($note->recommendation)
                                                        <p
                                                            class="mt-1 text-[11px] italic text-outline dark:text-zinc-500">
                                                            💡 {{ $note->recommendation }}
                                                        </p>
                                                    @endif
                                                </div>
        ...
        @if ($editMode)
            <div class="flex items-start gap-2 p-3 rounded-lg
                ...
                <flux:icon.pencil-square
                    class="size-4 text-violet-600 dark:text-violet-400 shrink-0 mt-0.5" />
                <div class="text-[11px] leading-relaxed
                            text-violet-800 dark:text-violet-300">
                    <span class="font-semibold">Editing progress report.</span>
                    After submission, the status will return to
                    <span class="font-semibold">Under Review</span>
                    and the same reviewer will be notified.
