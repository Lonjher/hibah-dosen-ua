...
new class extends Component
{
    ...
    public function addBudgetItem(): void
    {
        ...
        $current = (int) BudgetProposal::where('proposal_id', $this->draftProposalId)
            ->sum('amount');
        ...
        if (($current + $amount) > $limit) {
            ...
            $this->addError(
                'amount',
                'Exceeds budget limit. Maximum you can add: Rp ' .
    ...
    public function finish(): void
    {
        ...
        $total = (int) BudgetProposal::where('proposal_id', $this->draftProposalId)
            ->sum('amount');

        if ($total <= 0) {
            Flux::toast(
                text: 'Please add at least one budget item before finishing.',
                variant: 'danger'
            );
            return;
        }
        ...
        if ($total > $limit) {
            Flux::toast(
                text: 'Total budget exceeds the limit. Please review your items.',
                variant: 'danger'
            );
            return;
        }
    ...
    public function viewDetails(int $id): void
    {
        ...
        $this->dispatch(
            ...
            budgetPercent:   $budgetLimit > 0
                ? round(($budgetTotal / $budgetLimit) * 100, 1)
                : 0,
    ...
    public function confirmDelete(int $id): void
    {
        ...
        $this->dispatch(
            ...
            note: 'This action cannot be undone. All associated budget items will also be deleted.',
        );
    }
    ...
    {
        $this->reset([
            'step', 'editingId', 'draftProposalId',
            'research_scheme_id', 'title', 'summary', 'keywords', 'period_id',
            'item_name', 'amount',
        ]);
    ...
    public function with(): array
    {
        ...
        $query = Proposal::query()
            ...
            ->where('user_id', Auth::id())
            ->where('is_research', false)   // ← FORCED
            ...
            ->when($this->statusFilter !== 'all',
                fn ($q) => $q->where('status_proposal', $this->statusFilter))
            ->latest();
        ...
        if ($this->draftProposalId) {
            $proposal    = Proposal::with('researchScheme')->find($this->draftProposalId);
            $budgetItems = BudgetProposal::where('proposal_id', $this->draftProposalId)
                ->orderBy('id')
                ->get();
        ...
        return [
            ...
            'statusCounts' => Proposal::where('user_id', Auth::id())
                ->where('is_research', false)
                ->selectRaw('status_proposal, count(*) as total')
                ->groupBy('status_proposal')
                ->pluck('total', 'status_proposal'),

            'totalCount' => Proposal::where('user_id', Auth::id())
                ->where('is_research', false)
                ->count(),
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-16 text-center text-slate-500 dark:text-slate-400">
                                <div class="flex flex-col items-center gap-3">
                                    <flux:icon.hand-raised class="size-10 text-slate-300 dark:text-zinc-700" />
