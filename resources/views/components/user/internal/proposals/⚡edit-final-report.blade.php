...
new class extends Component {
    ...
    {
        $report = FinalReport::whereHas('proposal', fn ($q) => $q->where('user_id', auth()->id()))
            ->findOrFail($id);

        if (! in_array($report->status, ['pending', 'revised'])) {
            Flux::toast('Final report ini tidak dapat diedit.', variant: 'danger');
            return;
        }
        ...
        $this->dispatch('show-edit-final-report');
    }
    ...
    public function save(): void
    {
        ...
        $this->dispatch('final-report-updated');
        $this->resetAll();
    }
...
<div
    x-data="{
        ...
        errorMessage: '',
        init() {
            window.addEventListener('show-edit-final-report', () => {
                this.errorMessage = '';
                this.show = true;
            });
            window.addEventListener('final-report-error', (e) => { this.errorMessage = e.detail.message; });
            window.addEventListener('final-report-updated', () => { this.show = false; });
        }
    }"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        ...
                        @if ($existing_report_path)
                            <p class="mt-1 text-[10px] text-slate-500">File: {{ basename($existing_report_path) }}</p>
                        @endif
                    ...
                    <div>
                        ...
                        @if ($existing_ppt_path)
                            <p class="mt-1 text-[10px] text-slate-500">File: {{ basename($existing_ppt_path) }}</p>
                        @endif
                    ...
                    <div>
                        <label class="block text-[12px] font-medium text-slate-700 dark:text-zinc-300 mb-1">
                            Research Output <span class="text-slate-400">(opsional)</span>
                        </label>
                        ...
                        @if ($existing_research_output)
                            <p class="mt-1 text-[10px] text-slate-500">File: {{ basename($existing_research_output) }}</p>
                        @endif
                    ...
                    <div>
                        <label class="block text-[12px] font-medium text-slate-700 dark:text-zinc-300 mb-1">
                            Bukti Submit <span class="text-slate-400">(opsional)</span>
                        </label>
                        <input type="file" wire:model="submission_proof_file" accept="image/*"
                            class="block w-full text-[11px] text-slate-600 dark:text-zinc-400
                                   file:mr-2 file:py-1.5 file:px-3 file:rounded-md file:border-0
                                   file:text-[11px] file:font-medium file:bg-blue-50 file:text-blue-700" />
                        @if ($existing_submission_proof)
                            <p class="mt-1 text-[10px] text-slate-500">File: {{ basename($existing_submission_proof) }}</p>
                        @endif
